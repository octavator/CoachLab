...
var CoachAgenda = function (_React$Component) {
  ...
  _createClass(CoachAgenda, [{
    ...
    value: function showNextWeek() {
      var new_day = this.addDays(this.state.currentDay, 7);
      var new_week = this.getCurrentWeek(new_day);
    ...
    key: 'getCurrentWeek',
    value: function getCurrentWeek(targetDay) {
      var _this3 = this;

      var dayIndex = targetDay.getDay() - 1;
      dayIndex = dayIndex < 0 ? 6 : dayIndex;
      return this.state.weekdays.map(function (_day, i) {
        return _this3.addDays(targetDay, i - dayIndex);
      });
    }
  }, {
    key: 'toggleModal',
    ...
    key: 'buildResaId',
    value: function buildResaId(day, hour) {
      if (!this.state.currentWeek) return "null";
      var weekIdx = this.state.weekdays.indexOf(day);
      var date = this.state.currentWeek[weekIdx];
    ...
    value: function render() {
      ...
      var firstDate = this.state.currentWeek ? this.state.currentWeek[0].getDate() : "";
